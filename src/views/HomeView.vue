...
.stat-card.censored .v-card-text {
  filter: blur(8px);
  transition: filter 0.3s ease;
}

.stat-card.censored:hover .v-card-text {
  filter: blur(6px);
}
...
.evolution-bar-fill {
  width: 20px;
  background: linear-gradient(to top, var(--v-primary-base), var(--v-primary-lighten-1));
  border-radius: 3px 3px 0 0;
  transition: all 0.3s ease;
  cursor: pointer;
}

.evolution-bar-fill:hover {
  background: linear-gradient(to top, var(--v-primary-darken-1), var(--v-primary-base));
...
.member-item:hover,
.city-item:hover {
  background: var(--v-surface-variant-darken-1);
}
